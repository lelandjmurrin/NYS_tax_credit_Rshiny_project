...
```{r}
#Industry
industry_cleaned %>% filter(Name == 'Investment Tax Credit', Group == 'Manufacturing') %>% group_by(Name, Year, Group) %>% summarise(Num = sum(Num)) %>% arrange(desc(Year)) %>% head(5) %>% ungroup() %>% select(-c(Name,Group))
