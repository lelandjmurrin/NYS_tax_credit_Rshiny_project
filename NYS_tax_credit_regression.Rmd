...
for (a in 1:4){
  ...
  sat.model.bc <- lm(Avg.bc ~., data = xy.splits[[i]][['data.train']])
  
  b = baseline.list[a]
  temp.df <- as.data.frame(xy.splits[[b]][['X.test']]) #temp fix
  colnames(temp.df) <- str_replace_all(colnames(temp.df), "[`]", '')
  
  y.predict <- predict(sat.model.bc, newdata = temp.df)
  MSE.sat <- mean((y.predict - xy.splits[[b]][['y.test']])^2)
