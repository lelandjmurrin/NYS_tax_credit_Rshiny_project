...
user_input_df_updated <- user_input_df %>% mutate(value = ifelse(col == 'Alcoholic Beverage Production Credit', 1, value), value = ifelse(col == 'Year', 2024, value), value = ifelse(col == 'Num', 1, value), value = ifelse(col == '$1 - $99,999', 1, value)) %>% select(-col) %>% pivot_wider(names_from = dummy_col, values_from = value)
...
user_input_pred %>% sapply(boxcox_to_dollars, USE.NAMES = T)
...
```{r}
combined.true.pred <- combined.true.pred %>% write_csv('Shiny_app/data/combined.true.pred.csv')
```
...
income_cleaned %>% filter(Name == 'Investment Tax Credit', Year == 2019) %>% group_by(Year, Group) %>% summarise(sum(Num))
```
