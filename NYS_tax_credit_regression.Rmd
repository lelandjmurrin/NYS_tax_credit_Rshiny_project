...
test_train_split(income_cleaned, Avg ~ .)
...
regularization_func <- function (data, alpha, name){
  ...
  return(eval_results(y.test, lasso.best.lambda.train.pred, data.test)['Rsquare'])
}
...
all.splits <- list(
  ...
  'income.data.split.sat' = test_train_split(income.dummy.bc, Avg.bc ~ .),
  'income.data.split.best' = test_train_split(income.dummy.bc, income.best.formula)
)
#For loops initialization
formulas <- list(income_cleaned = sat.formula, income_cleaned_bc = sat.formula.bc, income.data.split.sat = sat.formula.bc, income.data.split.best = income.best.formula)
